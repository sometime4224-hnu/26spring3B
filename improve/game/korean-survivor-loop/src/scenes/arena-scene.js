(function () {
  class ArenaScene extends Phaser.Scene {
    ...
    create() {
      ...
      this.harvestRing = this.add.image(playerStartX, playerStartY, "weapon-blast-wave").setTint(0xa9f2bc).setAlpha(0).setDepth(3);
      this.supernovaRing = this.add.image(playerStartX, playerStartY, "weapon-blast-wave").setTint(0xffb6a3).setAlpha(0).setDepth(3);
      ...
      this.harvestOrbitals = Array.from({ length: 3 }, () => (
        this.add.image(playerStartX, playerStartY, "pickup-icon-score").setAlpha(0).setDepth(5)
      ));
    ...
    createLoopMapBackdrop() {
      ...
      this.mapBackdropDetail = this.add.tileSprite(0, 0, this.scale.width, this.scale.height, "loopmap-detail")
        ...
        .setDepth(0.3)
        .setAlpha(0.88);
      this.mapBackdropAccent = this.add.tileSprite(0, 0, this.scale.width, this.scale.height, "loopmap-detail")
        .setOrigin(0)
        .setScrollFactor(0)
        .setDepth(0.34)
        .setBlendMode(Phaser.BlendModes.ADD)
        .setAlpha(0.16);
      this.mapBackdropGlow = this.add.tileSprite(0, 0, this.scale.width, this.scale.height, "loopmap-base")
        .setOrigin(0)
        .setScrollFactor(0)
        .setDepth(0.12)
        .setBlendMode(Phaser.BlendModes.ADD)
    ...
    updateLoopMapBackdrop() {
      ...
      if (this.mapBackdropAccent) {
        this.mapBackdropAccent.tilePositionX = this.player.x * 0.46;
        this.mapBackdropAccent.tilePositionY = this.player.y * 0.46;
      }

      if (this.mapBackdropGlow) {
        this.mapBackdropGlow.tilePositionX = this.player.x * -0.22;
        this.mapBackdropGlow.tilePositionY = this.player.y * -0.18;
        const glowBaseAlpha = this.mapBackdropGlowBaseAlpha == null ? 0.08 : this.mapBackdropGlowBaseAlpha;
        this.mapBackdropGlow.setAlpha(glowBaseAlpha + Math.abs(Math.sin(this.time.now * 0.0012)) * 0.04);
      }
    }
    ...
    applyStageBackdropTheme() {
      ...
      if (this.mapBackdropBase) {
        ...
        this.mapBackdropBase.setAlpha(loopTheme.baseAlpha == null ? 0.9 : loopTheme.baseAlpha);
      }

      if (this.mapBackdropDetail) {
        this.mapBackdropDetail.setTint(loopTheme.detailTint == null ? 0xaad2e0 : loopTheme.detailTint);
        this.mapBackdropDetail.setAlpha(loopTheme.detailAlpha == null ? 0.88 : loopTheme.detailAlpha);
      }

      if (this.mapBackdropAccent) {
        this.mapBackdropAccent.setTint(loopTheme.accentTint == null ? 0xffc884 : loopTheme.accentTint);
        this.mapBackdropAccent.setAlpha(loopTheme.accentAlpha == null ? 0.16 : loopTheme.accentAlpha);
      }

      if (this.mapBackdropGlow) {
        this.mapBackdropGlow.setTint(loopTheme.glowTint == null ? 0x8ad3ff : loopTheme.glowTint);
        this.mapBackdropGlowBaseAlpha = loopTheme.glowAlpha == null ? 0.08 : loopTheme.glowAlpha;
        this.mapBackdropGlow.setAlpha(this.mapBackdropGlowBaseAlpha);
      }
    }
